@php
    ...
    $enableToggleSideMenuButton =
        app()->has('themes.menu_sidebar') &&
        app('themes.menu_sidebar') &&
        Arr::get($config, 'enable_toggle_side_menu_button', true);
        ...
        <div class="action-buttons-inner d-flex align-items-center">
                        ...
                        <div class="search-input-area">
                            <div class="container">
                                <form action="{{ route('public.search') }}" class="search-input-inner">
                                    <div class="input-div">
                                        <input id="searchInput1" class="search-input" name="q" type="text"
                                            placeholder="{{ __('Search by keyword') }}">
                                    </div>
                                    <div class="search-close-icon"><i class="fa-regular fa-xmark-large rt-xmark"></i>
                                    </div>
                                </form>
                            </div>
                        </div>
            ...
            @if ($enableToggleSideMenuButton)
                @include(Theme::getThemeNamespace(
                        'widgets.action-buttons.templates.partials.toggle-sidebar-menu-button'))
            @endif
        </div>
    </div>
@endif
